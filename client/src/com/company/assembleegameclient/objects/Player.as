...
public class Player extends Character {
    public static const MS_BETWEEN_TELEPORT:int = 100;
    ...
    public var airStorage_:int = 0;
    public var waterStorage_:int = 0;
    public var fireStorage_:int = 0;
    public var earthStorage_:int = 0;
    ...
    public var lockerLootbox_:int = 0;
    ...
    protected var empoweredEffect_:HealingEffect = null;
    ...
    public static var correctServer:int = 1;
    public static var _atk:int;
    public static var _def:int;
    public static var _spd:int;
    public static var _dex:int;
    public static var _vit:int;
    public static var _wis:int;
    public static var _firerate:int;
    public static var _mindmg:int;
    public static var _maxdmg:int;
    public var APotions:int = 0;
    public var DPotions:int = 0;
    public var SPotions:int = 0;
    public var DEPotions:int = 0;
    public var VPotions:int = 0;
    public var WPotions:int = 0;
    public var LPotions:int = 0;
    public var MPotions:int = 0;
    public var LUPotions:int = 0;
    public var RPotions:int = 0;
    ...
    private var abilityOngoing:Boolean = false;
    ...
    public static function fromPlayerXML(name:String, plrXML:XML):Player {
        ...
        plr.restoration_ = int(plrXML.Restoration);
    ...
    public function setEventLootbox(box:int):void {
        this.eventLootbox_ = box;
    }
    ...
    public function setPartyId(_arg1:int) : void
    {
        ...
        if(_loc5_ != this)
        {
            _loc4_ = _loc5_ != null && _loc5_.partyId_ != -1 && _loc5_.partyId_ == this.partyId_;
            if(_loc4_ != this.isPartyMember_)
            {
                this.isPartyMember_ = _loc4_;
                nameBitmapData_ = null;
            }
        }
        else
        {
            for each(_loc2_ in map_.goDict_)
            {
                ...
                {
                    _loc3_.setPartyId(_loc3_.partyId_);
                }
            }
        }
    }
    ...
    override public function update(currentTime:int, deltaMS:int):Boolean {
        map_.gs_.gsc_.correctServerCheck();
        ...
        if (isHealing() && !isPaused()
            ...
            this.healingEffect_ = new HealingEffect(this, 0xE03434);
        ...
        if (isEmpowered() && !isPaused()
                && this.empoweredEffect_ == null
                && !Parameters.data_.noParticlesMaster) {
            this.empoweredEffect_ = new HealingEffect(this, 0x6084E0);
            map_.addObj(this.empoweredEffect_, x_, y_);
        }
        else {
            if (this.empoweredEffect_ != null) {
                map_.removeObj(this.empoweredEffect_.objectId_);
                this.empoweredEffect_ = null;
            }
        }
        ...
        if (map_.player_ == this &&
                ...
                (square_.lastDamage_ + 250) < currentTime &&
                !isInvincible() &&
            ...
            damage(-1, dmg / 2, conditionEffects, hp_ <= dmg / 2 && this.protectionPoints_ == 0, null);
    ...
    private function getNameColor():uint {
        ...
        if (this.isPartyMember_) {
            return (Parameters.FELLOW_PARTY_COLOR);
        }
        return (0xFFFFFF);
    }
    ...
    private function getMoveSpeed():Number {
        ...
        if (isSlowed()) {
            _local1 = (MIN_MOVE_SPEED + ((this.speed_ / 75 / 4) * (MAX_MOVE_SPEED - MIN_MOVE_SPEED)));
        }
    ...
    public function attackFrequency():Number {
        ...
        _firerate = this.attackPeriod_;
        _mindmg = square_.props_.minDamage_;
        _maxdmg = square_.props_.maxDamage_;
        var _local1:Number = 1;
        if (isDazed()) {
            _local1 = (MIN_ATTACK_FREQ + (((this.dexterity_) / 75 / 4) * (MAX_ATTACK_FREQ - MIN_ATTACK_FREQ)));
        } else {
            _local1 = (MIN_ATTACK_FREQ + (((this.dexterity_) / 75) * (MAX_ATTACK_FREQ - MIN_ATTACK_FREQ)));
        }
    ...
    private function attackMultiplier():Number {
        var _local1:Number = 1;
        if (isWeak()) {
            _local1 = (MIN_ATTACK_MULT + (this.attack_ / 50 / 4));
        } else {
            _local1 = (MIN_ATTACK_MULT + (this.attack_ / 50));
        }
    ...
    private function isMercy():Boolean{
        if(ObjectLibrary.typeToDisplayId_[this.equipment_[2]] == "Grace of the Colossus" && mp_ >= maxMP_/2){
            return true;
        }else{
            return false;
        }
    ...
    public function useAltWeapon(mouseX:Number, mouseY:Number, useType:int):Boolean {
        ...
        var multiPhase:Boolean = itemXML.hasOwnProperty("MultiPhase");
        if (useType == UseType.START_USE && !multiPhase) {
        ...
        else if (this.abilityOngoing && multiPhase && useType == UseType.END_USE) {
            cooldownMS = 550;
            if (itemXML.hasOwnProperty("Cooldown")) {
                cooldownMS = (Number(itemXML.Cooldown) * 1000);
            }
            ...
            for each (activateXML in itemXML.Activate) {
                if (activateXML.toString() == ActivationType.SHURIKEN_ABILITY) {
                    shootAngle = Math.atan2(mouseY, mouseX);
                    this.doShoot(curTime, itemId, itemXML, (Parameters.data_.cameraAngle + shootAngle), false);
                }
            }
        ...
        else if (multiPhase && !this.abilityOngoing) {
    ...
    public function attemptAttackAngle(angle:Number, unstable:Boolean):void {
        var offset:Number = angle;
        if(unstable)
            offset += Math.random() - 0.4;
        this.shoot(Parameters.data_.cameraAngle + offset);
    }
    ...
    private function shoot(angle:Number):void {
        ...
        //this.addTextLine.dispatch(ChatMessage.make(Parameters.HELP_CHAT_NAME, "1: " + String(int(true))));
    ...
    private function doShoot(_arg1:int, _arg2:int, _arg3:XML, _arg4:Number, _arg5:Boolean):void {
        ...
        while (_local10 < _local6) {
            ...
            dmgMin = int(_local12.projProps_.minDamage_);
            dmgMax = int(_local12.projProps_.maxDamage_);
            dmgMulti = _arg5 ? this.attackMultiplier() : 1;
            dmg = map_.gs_.gsc_.getNextDamage(dmgMin, dmgMax);
            critRoll = map_.gs_.gsc_.getNextDamage(1, 1001);
            isCrit = isBravery() && IsCritical(critRoll);
            dmg = GetDamage(dmg, dmgMulti, isCrit);
            if (_arg1 > (map_.gs_.moveRecords_.lastClearTime_ + 600))
                dmg = 0;
            _local12.setDamage(dmg, isCrit, CriticalMulti());
    ...
    private function GetDamage(dmg:Number, multi:Number, crit:Boolean):Number {
        var damage:Number = dmg;
        damage *= multi;
        if(ObjectLibrary.typeToDisplayId_[this.equipment_[3]] == "Band of Exoneration" && hp_ >= maxHP_ * 0.80)
            damage *= 1.3;
        if(mark_ == 3 && surge_ < 50)
            damage *= (2.0 - surge_ / 50);
        if(crit)
    ...
    private function IsCritical(roll:int):Boolean {
    ...
    private function CriticalMulti():Number {
        return 2;
    }

    private function BonusDamage():Number {
        var bonus:Number = 0;
        if(ObjectLibrary.typeToDisplayId_[this.equipment_[0]] == "Evanescence" && isInvisible() == true)
            bonus += 20 + wisdom_*5;
        if(isVengeance())
            bonus += (maxHP_-hp_)/2;
        if(isRelentless())
