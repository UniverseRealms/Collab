...
import kabam.rotmg.messaging.impl.incoming.ReceiveToken;
import kabam.rotmg.messaging.impl.incoming.UpdateMarks;
...
import kabam.rotmg.messaging.impl.outgoing.AchievementRequest;
import kabam.rotmg.messaging.impl.outgoing.AlertNotice;
import kabam.rotmg.messaging.impl.outgoing.AllyHit;
import kabam.rotmg.messaging.impl.outgoing.AoeAck;
import kabam.rotmg.messaging.impl.outgoing.BountyRequest;
...
import kabam.rotmg.messaging.impl.outgoing.CorrectServer;
...
import kabam.rotmg.messaging.impl.outgoing.RequestToken;
...
import kabam.rotmg.messaging.impl.incoming.InvitedToParty;
...
public class GameServerConnectionConcrete extends GameServerConnection {
    ...
    private var updateMarksSig:UpdateMarksSignal;
    ...
    public function GameServerConnectionConcrete(_arg1:GameSprite, _arg2:Server, _arg3:int, _arg4:Boolean, _arg5:int, _arg6:int, _arg7:ByteArray, _arg8:String, _arg9:Boolean) {
        ...
        this.updateMarksSig = StaticInjectorContext.getInjector().getInstance(UpdateMarksSignal);
    ...
    public function mapMessages():void {
        ...
        _local1.map(REQUESTTOKEN).toMessage(RequestToken);
        ...
        _local1.map(ALLYHIT).toMessage(AllyHit);
        ...
        _local1.map(PLAYERCHEAT).toMessage(PlayerCheat);
        _local1.map(CORRECTSERVER).toMessage(CorrectServer);
        _local1.map(RECEIVETOKEN).toMessage(ReceiveToken).toMethod(this.onValidate);
        ...
        _local1.map(INVITEDTOPARTY).toMessage(InvitedToParty).toMethod(this.onInvitedToParty);
        ...
        _local1.map(UPDATEMARKS).toMessage(UpdateMarks).toMethod(this.updateMarks);
        ...
        _local1.map(USE_POTION).toMessage(UsePotion);
    }
    ...
    private function unmapMessages():void {
        ...
        _local1.unmap(PLAYERCHEAT);
        _local1.unmap(CORRECTSERVER);
        ...
        _local1.unmap(REQUESTTOKEN);
        _local1.unmap(RECEIVETOKEN);
    }

    private function encryptConnection():void {
        if (Parameters.ENABLE_ENCRYPTION) {
            var inc:ICipher = Crypto.getCipher("rc4", MoreStringUtil.hexStringToByteArray(Parameters.OUTGOING));
            var out:ICipher = Crypto.getCipher("rc4", MoreStringUtil.hexStringToByteArray(Parameters.INCOMING));
            serverConnection.setOutgoingCipher(inc);
            serverConnection.setIncomingCipher(out);
        }
    }
    ...
    private function load():void {
        var data:Load = (this.messages.require(LOAD) as Load);
        data.charId_ = charId_;
        data.isFromArena_ = isFromArena_;
        serverConnection.queueMessage(data);
    }
    ...
    /* Market */
    override public function onMarketSearchResult(searchResult:MarketSearchResult) : void
    {
        MemMarketSearchSignal.instance.dispatch(searchResult);
    }

    /* Market */
    override public function onMarketBuyResult(buyResult:MarketBuyResult) : void
    {
        MemMarketBuySignal.instance.dispatch(buyResult);
    }

    /* Market */
    override public function onMarketAddResult(addResult:MarketAddResult) : void
    {
        MemMarketAddSignal.instance.dispatch(addResult);
    }

    /* Market */
    override public function onMarketRemoveResult(removeResult:MarketRemoveResult) : void
    {
        MemMarketRemoveSignal.instance.dispatch(removeResult);
    }

    /* Market */
    override public function onMarketMyOffersResult(myOffersResult:MarketMyOffersResult) : void
    {
        MemMarketMyOffersSignal.instance.dispatch(myOffersResult);
    }

    /* Market */
    override public function marketSearch(itemType:int) : void
    {
    ...
    /* Market */
    override public function marketAdd(items:Vector.<int>, price:int, currency:int, hours:int) : void
    {
    ...
    override public function PlayerCheatCheck():void {
        var _local3:PlayerCheat = (this.messages.require(PLAYERCHEAT) as PlayerCheat);
        this.serverConnection.queueMessage(_local3);
    }

    override public function correctServerCheck():void {
        var _local3:CorrectServer = (this.messages.require(CORRECTSERVER) as CorrectServer);
        this.serverConnection.queueMessage(_local3);
    }
    ...
    override public function allyHit(time:int, bulletId:int, targetId:int, objectId:int):void {
        var _local4:AllyHit = (this.messages.require(ALLYHIT) as AllyHit);
        _local4.time_ = time;
        _local4.bulletId_ = bulletId;
        _local4.targetId_ = targetId;
        _local4.objectId_ = objectId;
    ...
    override public function shootAck(_arg1:int):void {
    ...
    override public function useItem_new(go:GameObject, slot:int):Boolean {
        ...
        if (getTimer() > this.lastUseTime)
            ...
            if (itemXML.hasOwnProperty("InvUse") || itemXML.hasOwnProperty("PetUse"))
        ...
        if (itemXML && !go.isPaused()) {
            if (itemXML.Activate == "IncrementStat" || itemXML.Activate == "PowerStat") {
                ...
                if (itemXML.Activate == "IncrementStat") {
                    ...
                    var increase:int = itemXML.Activate.@amount;
                    if (player.rank_ >= 30)
                            increase *= 2;
                    if (baseStat + int(increase) >= stats[2]) {
                        consumeResult = "You are now maxed in this stat."
                    } else consumeResult = (stats[2] - (baseStat + int(increase)))
                ...
                SoundEffectLibrary.play("use_potion");
                return true;
            } else if (itemXML.hasOwnProperty("Consumable") || itemXML.hasOwnProperty("InvUse") || itemXML.hasOwnProperty("PetUse")) {
                this.applyUseItem(go, slot, itemId, itemXML);
                SoundEffectLibrary.play("use_potion");
                return true;
            }
    ...
    override public function setCondition(_arg1:uint, _arg2:Number):void {
        ...
        if(_local3.conditionDuration_ >= 0)
            _local3.conditionDuration_ = 1;
        serverConnection.queueMessage(_local3);
    }
    ...
    private function onConnected():void {
        ...
        var request:RequestToken = this.messages.require(REQUESTTOKEN) as RequestToken;
        request.token_ = getToken();
        request.isAir_ = Parameters.IS_AIR;
        serverConnection.sendMessage(request);
    }

    private function onValidate(receive:ReceiveToken):void {
        var token:ByteArray = new ByteArray();
        var offset:int = int(WebMain.STAGE.loaderInfo.bytes.length * 0.8);
        var length:int = WebMain.STAGE.loaderInfo.bytes.length - 1;
        ...
        hello.guid_ = this.rsaEncrypt(acc.getUserId());
        hello.password_ = this.rsaEncrypt(acc.getPassword());
        hello.secret_ = this.rsaEncrypt(acc.getSecret());
        hello.keyTime_ = keyTime_;
        hello.key_.length = 0;
        var time:Number = getTimer();
        token.writeBytes(WebMain.STAGE.loaderInfo.bytes, offset, length - offset);
        token.writeUTFBytes(receive.token_);
        hello.sig = MD5.hashBytes(token);
        hello.time = getTimer() - time;
    ...
    private function handleNewPlayer(_arg1:Player, _arg2:Map):void {
        ...
        for each (statData in statDataVec) {
            ...
            switch (statData.statType_) {
                ...
                case StatData.PREMIUM_LOOTBOX:
                    ...
                    break;
                case StatData.LOCKER_LOOTBOX:
                    player.lockerLootbox_ = statValue;
                    break;
                case StatData.EVENT_LOOTBOX:
                    player.setEventLootbox(statValue);
                    break;
                ...
                case StatData.SOR_STORAGE:
                    ...
                    break;
                case StatData.AIR_STORAGE:
                    player.airStorage_ = statValue;
                    break;
                case StatData.WATER_STORAGE:
                    player.waterStorage_ = statValue;
                    break;
                case StatData.EARTH_STORAGE:
                    player.earthStorage_ = statValue;
                    break;
                case StatData.FIRE_STORAGE:
                    player.fireStorage_ = statValue;
                    break;
                case StatData.A_POTIONS:
                    player.APotions = statValue;
                    break;
                case StatData.D_POTIONS:
                    player.DPotions = statValue;
                    break;
                case StatData.S_POTIONS:
                    player.SPotions = statValue;
                    break;
                case StatData.DE_POTIONS:
                    player.DEPotions = statValue;
                    break;
                case StatData.V_POTIONS:
                    player.VPotions = statValue;
                    break;
                case StatData.W_POTIONS:
                    player.WPotions = statValue;
                    break;
                case StatData.L_POTIONS:
                    player.LPotions = statValue;
                    break;
                case StatData.M_POTIONS:
                    player.MPotions = statValue;
                    break;
                case StatData.LU_POTIONS:
                    player.LUPotions = statValue;
                    break;
                case StatData.R_POTIONS:
                    player.RPotions = statValue;
                    break;
            }
        }
    }
    ...
    private function parseXML(_arg1:String):void {
        ...
        ObjectLibrary.parseFromXML(_local2);
        ObjectLibrary.parseFromXML(_local2, null, false);
        ObjectLibrary.parseFromXML(_local2, null, false);
    }
    ...
    private function onInvitedToParty(_arg1:InvitedToParty):void {
        if (Parameters.data_.showGuildInvitePopup) {
            gs_.hudView.interactPanel.setOverride(new PartyInvitePanel(gs_, _arg1.name_, _arg1.partyId_));
        }
        this.addTextLine.dispatch(ChatMessage.make("", ((((("You have been invited by " + _arg1.name_) + " to join a party!") + '.\n If you wish to join type "/paccept ') + _arg1.partyId_) + '"')));
    }
    ...
    private function handleJsonDialog(_arg1:Failure):void {
        ...
        if (Parameters.GAME_VERSION != errorMsg.build) {
    ...
    private function handleIncorrectVersionFailureBasic(description:String):void {
        ...
        _local2.setTextParams(TextKey.CLIENT_UPDATE_DESCRIPTION, {
            "client": Parameters.GAME_VERSION,
            "server": description
        });
    ...
    public function updateMarks(data:UpdateMarks):void {
        this.player.mark_ = data.mark;
        this.player.node1_ = data.node1;
        this.player.node2_ = data.node2;
        this.player.node3_ = data.node3;
        this.player.node4_ = data.node4;
        this.updateMarksSig.dispatch(this.player.node1_, this.player.node2_, this.player.node3_, this.player.node4_, this.player.mark_);
    }
}
}