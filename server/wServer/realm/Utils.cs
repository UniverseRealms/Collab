    ...
    internal static class EntityUtils
    {
        ...
        public static IEnumerable<Entity> GetNearestEntitiesBySquare(this Entity entity, double dist, ushort? objType, bool seeInvis = false)   //Null for player
        {
            ...
            if (objType == null)
                foreach (var i in entity.Owner.PlayersCollision.HitTest(entity.X, entity.Y, dist).Where(e => e is Player))
                {
                    if (!seeInvis && !(i as Player).IsVisibleToEnemy()) 
                        continue;
        ...
        public static int CountEntity(this Entity entity, double dist, ushort? objType)
        {
            ...
            if (objType == null)
                ...
                {
                    if (!(i as Player).IsVisibleToEnemy()) 
                        continue;
        ...
        {
            List<Entity> ens = new List<Entity>();
            if (players)
            {
                foreach (var i in entity.Owner.PlayersCollision.HitTest(entity.X, entity.Y, radius).Where(e => e is Player))
                {
        ...
        public static void AOETrap(this Entity entity, float radius, bool players, Action<Entity> callback)   //Null for player
        {
            List<Entity> ens = new List<Entity>();
            if (players)
            {
            ...
            else
            {
                foreach (var i in entity.Owner.EnemiesCollision.HitTest(entity.X, entity.Y, radius))
                {
                    if (!(i is Enemy))
                        continue;
                    if (i.HasConditionEffect(ConditionEffects.Invulnerable) || i.HasConditionEffect(ConditionEffects.Invincible))
                        continue;
        ...
        public static void AOE(this World world, Position pos, float radius, int damage, Entity src, bool noDef = false, ConditionEffect[] effs = null)
        {
            if (src == null)
                return;
            List<Entity> ens = new List<Entity>();
            ...
            if (!(src is Player)) {
                foreach (var i in world.PlayersCollision.HitTest(pos.X, pos.Y, radius).Where(e => e is IPlayer))
                {
                    var d = MathsUtils.DistSqr(i.X, i.Y, pos.X, pos.Y);
                    if (d < r2)
                        ens.Add(i);
                }
                foreach (IPlayer p in ens) {
                    p.Damage(damage, src as Enemy, noDef);
                    (p as Entity).ApplyConditionEffect(effs); // todo: Ally objects throw errors when given some condition effects
                }
            } else {
                foreach (var i in world.EnemiesCollision.HitTest(pos.X, pos.Y, radius))
                {
                ...
                foreach (Enemy en in ens)
                    en.Damage(src as IPlayer, damage, noDef, effs);
            }
        }

        public static Ally GetNearestAlly(this Entity entity, double dist, ushort? objType, bool seeInvis = false, Predicate<Entity> predicate = null)
        {
            if (entity.Owner == null) return null;
            Ally ret = null;
            
            var allies = entity.GetNearestAllies(dist, objType, seeInvis).ToArray();
            if (allies.Length == 0)
                return null;
                
            foreach (Ally i in allies)
            {
                if ((!(i as Ally).IsVisibleToEnemy() && !seeInvis) ||
                    i == entity ||
                    (objType != null && i.ObjectType != objType))
                    continue;
        ...
        public static IEnumerable<Ally> GetNearestAllies(this Entity entity, double dist, ushort? objType, bool seeInvis = false)   //Null for player
        {
            if (entity.Owner == null) yield break;
            foreach (var i in entity.Owner.PlayersCollision.HitTest(entity.X, entity.Y, dist).Where(e => e is Ally))
            {
                if (objType != null && i.ObjectType != objType)
                    continue;
                if (!seeInvis && !(i as IPlayer).IsVisibleToEnemy()) continue;
                var d = i.Dist(entity);
                if (d < dist)
                    yield return (i as Ally);
            }
        }
        ...
        public static void ForceUpdate(this Entity e, int slot)
        {
            ...
            switch (slot)
            {
                ...
                case 10:
                case 11:
                    e.InvokeStatChange((StatsType)((int)StatsType.Inventory0 + slot), (e as IContainer).Inventory[slot]?.ObjectType ?? -1);
                    break;
                ...
                case 18:
                case 19:
                    e.InvokeStatChange((StatsType)((int)StatsType.BackPack0 + slot), (e as IContainer).Inventory[slot]?.ObjectType ?? -1);
                    break;

                case 254:
                    e.InvokeStatChange(StatsType.HealthStackCount, (e as Player).HealthPots.Count);
                    break;

                case 255:
                    e.InvokeStatChange(StatsType.MagicStackCount, (e as Player).MagicPots.Count);
                    break;
            }
        }
    }
