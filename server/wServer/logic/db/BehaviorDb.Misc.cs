...
namespace wServer.logic
{
    partial class BehaviorDb
    {
        private _ Misc = () => Behav()
           .Init("TEST Laser Beam Signal",
                new State(
                    new State("wait",
                        new Follow(0.45, 20, 0),
                        new PlayerWithinTransition(0.5, "bringDownLightning"),
                        new TimedTransition(1500, "warning")
                      ),
                    new State("warning",
                        new Flash(0xFFFFFF, 0.2, 10),
                        new Follow(0.45, 20, 0),
                        new PlayerWithinTransition(0.5, "bringDownLightning"),
                        new TimedTransition(1500, "bringDownLightning")
                        ),
                    new State("bringDownLightning",
                        new Transform("TEST Laser Beam")
                )
            )
            )
            .Init("TEST Laser Beam",
                new State(
                    new State("1",
                        new TimedTransition(200, "2")
                      ),
                    new State("2",
                        new EnemyAOE(1, players: false, minDamage: 150, maxDamage: 150, noDef: true, color: 0xb2b2ff),
                        new Decay(350)
                )
            )
            )
