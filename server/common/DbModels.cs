...
namespace common
{
    public abstract class RedisObject
    {
        ...
        {
            byte[] buff;
            if (typeof(T) == typeof(int) || typeof(T) == typeof(uint) || typeof(T) == typeof(ushort) || typeof(T) == typeof(string))
        ...
        private void ReadyFlush()
        {
            ...
            foreach (var update in _update.Where(entry => entry != null && !entry.Name.IsNullOrEmpty && _entries.ContainsKey(entry.Name)))
    ...
    public class DbAccount : RedisObject
    {
        ...
        public int PartyId
        {
            get { return GetValue<int>("partyId"); }
            set { SetValue<int>("partyId", value); }
        }

        public int PartyRank
        {
            get { return GetValue<int>("partyRank"); }
            set { SetValue<int>("partyRank", value); }
        }
        ...
        {
            get => GetValue<int>("airStorage");
            set => SetValue("airStorage", value);
        }
        public int WaterStorage
        {
            get => GetValue<int>("waterStorage");
            set => SetValue("waterStorage", value);
        }
        public int EarthStorage
        {
            get => GetValue<int>("earthStorage");
            set => SetValue("earthStorage", value);
        }
        public int FireStorage
        {
            get => GetValue<int>("fireStorage");
            set => SetValue("fireStorage", value);
        }

        public int ChanceDenom
        {
            get => GetValue<int>("ChanceDenom");
            set => SetValue("ChanceDenom", value);
        }

        public int PotionStorage
        {
            get => GetValue<int>("PotionStorage");
            set => SetValue("PotionStorage", value);
        }

        public int StoredLife
        {
            get => GetValue<int>("StoredLife");
            set => SetValue("StoredLife", value);
        }

        public int StoredMana
        {
            get => GetValue<int>("StoredMana");
            set => SetValue("StoredMana", value);
        }

        public int StoredAttack
        {
            get => GetValue<int>("StoredAttack");
            set => SetValue("StoredAttack", value);
        }

        public int StoredDefense
        {
            get => GetValue<int>("StoredDefense");
            set => SetValue("StoredDefense", value);
        }

        public int StoredSpeed
        {
            get => GetValue<int>("StoredSpeed");
            set => SetValue("StoredSpeed", value);
        }

        public int StoredDexterity
        {
            get => GetValue<int>("StoredDexterity");
            set => SetValue("StoredDexterity", value);
        }

        public int StoredVitality
        {
            get => GetValue<int>("StoredVitality");
            set => SetValue("StoredVitality", value);
        }

        public int StoredWisdom
        {
            get => GetValue<int>("StoredWisdom");
            set => SetValue("StoredWisdom", value);
        }

        public int StoredLuck
        {
            get => GetValue<int>("StoredLuck");
            set => SetValue("StoredLuck", value);
        }

        public int StoredRestoration
        {
            get => GetValue<int>("StoredRestoration");
            set => SetValue("StoredRestoration", value);
        }
        ...
        {
            get => GetValue<int>("lootbox6");
            set => SetValue("lootbox6", value);
        }

        public int EventLootbox
        {
            get => GetValue<int>("lootbox7");
            set => SetValue("lootbox7", value);
        }

        public int CheckCheats
        {
            get => GetValue<int>("cheatCount");
            set => SetValue<int>("cheatCount", value);
        }
        ...
        public PrivateMessages PrivateMessages
        {
            get
            {
                ...
                return pMessages != null ? Utils.FromJson<PrivateMessages>(pMessages) : null;
            }
    ...
    public class DbMarketData
    {
        ...
        internal DbMarketData(IDatabase db, int id)
        {
            ...
            Id = id;
            var json = (string)db.HashGet("memMarket", id);
            if (json == null)
            {
                IsNull = true;
            }
        ...
        public static int NextId(IDatabase db)
        {
            ...
            } while ((string)db.HashGet("memMarket", id) != null);
            return id;
        }
        ...
        {
            var allOffers = db.Conn.HashGetAll("memMarket");
            foreach (var i in allOffers)
            {
                DbMarketData l = JsonConvert.DeserializeObject<DbMarketData>(i.Value);
    ...
    public class DbNews // TODO. Check later, range results might be bugged...
    {
        ...
        {
            Entries = db.SortedSetRangeByRankWithScores("news", 0, 10).Select(x =>
                {
                    var ret = JsonConvert.DeserializeObject<DbNewsEntry>(Encoding.UTF8.GetString(x.Element));
    ...
    public class DbVault : RedisObject
    {
        ...
        {
            get => GetValue<ushort[]>("vault." + index) ?? Enumerable.Repeat((ushort)0xffff, 8).ToArray();
    ...
    public static class DbLegend
    {
        ...
        public static void Clean(IDatabase db)
        {
            foreach (var span in TimeSpans)
            {
                ...
                {
                    db.SortedSetRemoveRangeByRankAsync($"legends:{span.Key}:byFame", 0, -MaxListings - 1, CommandFlags.FireAndForget);
                    continue;
                }
            ...
            foreach (var span in TimeSpans)
            {
                var legendTask = db.SortedSetRangeByRankAsync($"legends:{span.Key}:byFame", 0, MaxGlowingRank - 1, Order.Descending);
    ...
    public class DbParty : RedisObject
    {
        internal DbParty(IDatabase db, int id)
        {
            Id = id;
            Init(db, "party." + id);
        }

        public DbParty(DbAccount acc)
        {
            Id = acc.PartyId;
            Init(acc.Database, "party." + Id);
        }
        ...
        {
            get { return GetValue<string>("leader") ?? ""; }
            set { SetValue<string>("leader", value); }
        }
    ...
    public class DbMarket
    {
        ...
        public DbMarket(IDatabase db)
        {
            ...
            var entries = db.HashGetAll(_key).Select(x => new PlayerShopItem(
        ...
        public Task<bool> InsertAsync(PlayerShopItem shopItem, ITransaction transaction = null)
        {
            ...
            var task = trans.HashSetAsync(_key, shopItem.Id, buff).ContinueWith(t => Item2List(shopItem, t, true));

            task.ContinueWith(e => Log.Error(e.Exception.InnerException.ToString()), TaskContinuationOptions.OnlyOnFaulted);
        ...
        public Task<bool> RemoveAsync(PlayerShopItem shopItem, ITransaction transaction = null)
        {
            ...
            var task = trans.HashDeleteAsync(_key, shopItem.Id).ContinueWith(t => Item2List(shopItem, t, false));
        ...
        private bool Item2List(PlayerShopItem shopItem, Task<bool> t, bool add)
        {
            ...
            if (success)
            {
                if (Monitor.TryEnter(ListLock, new TimeSpan(0, 0, 1)))
                    try
                    {
                        if (add)
        ...
        {
            if (Monitor.TryEnter(ListLock, new TimeSpan(0, 0, 1)))
                try { return _entries.SingleOrDefault(e => e.Id == id); }
                finally { Monitor.Exit(ListLock); }
    ...
    public class DbTinker
    {
        ...
        public Task<bool> InsertAsync(Tinker quest, ITransaction transaction = null)
        {
            ...
            var task = trans.HashSetAsync(KEY, quest.DbId, buff).ContinueWith(t => UpdateList(quest, t, true));
        ...
        private Task<bool> DeleteAsync(Tinker quest, ITransaction transaction = null)
        {
            ...
            var task = trans.HashDeleteAsync(KEY, quest.DbId).ContinueWith(t => UpdateList(quest, t, false));
        ...
        {
            if (Monitor.TryEnter(listLock, new TimeSpan(0, 0, 1)))
                try { return entries.FirstOrDefault(_ => _.OwnerId == accountId); }
                finally { Monitor.Exit(listLock); }
